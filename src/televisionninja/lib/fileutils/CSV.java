...
public class CSV {
	...
	public static String parseToCSV(final List<String> values, char separator, char customQuote) {
		if (customQuote == ' ') {
			customQuote = new CSV().getQuote();
		}

		if (separator == ' ') {
			separator = new CSV().getSeparator();
		}
	...
	public CSV(final File csv) {
		if (FileName.getExtension_2(csv.getAbsolutePath()).equalsIgnoreCase(this.extension)) {
			this.csv = csv;
			refreshTemp();
		}
		else {
			System.out.println("CSV() not a CSV file");
		}
	}
	...
	public void rename(final String newName) {
		if (FileName.getExtension_2(newName).equalsIgnoreCase(this.extension)) {
			...
			System.out.println("rename() not a csv file");
		}
	}
	...
	public void setCSV(final File csv) {
		if (FileName.getExtension_2(csv.getAbsolutePath()).equalsIgnoreCase(this.extension)) {
			...
			System.out.println("setCSV() not a csv file");
		}
	}
